<x-layout.apps>
    <x-slot name="header">
        <x-slot name="header">
            <button
                onclick="window.location.href='{{ route('dashboard.bulan.show', encrypt($transactionWifiItem->transaction_wifi_id)) }}'"
                ...
                <div
                    ...
                    <div>
                        ...
                        <h2 class="my-2 text-2xl font-semibold text-gray-700 dark:text-gray-200">
                            {{ __('Edit Transaction Wifi Item') }} -- {{ $transactionWifiItem->id }}
                        </h2>
                        <hr class="my-2">
                        <form action="{{ route('dashboard.item.update', encrypt($transactionWifiItem->id)) }}"
                                ...
                                <select name="users_id" id="users_id"
                                    ...
                                    @foreach ($users as $user)
                                        ...
                                        <option value="{{ $user->id }}"
                                            {{ old('users_id', $user->id) == $transactionWifiItem->users_id ? 'selected' : '' }}>
                                            {{ $user->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="transactions_id" id="transactions_id" class="select-id-transaksi w-full">
                                    ...
                                    @foreach ($transactions as $tf)
                                        @if ($tf->status == 'SUCCESS')
                                            <option value="{{ $tf->id }}"
                                                {{ old('transactions_id', $tf->id) == $transactionWifiItem->wifis->transactions_id ? 'selected' : '' }}>
                                                {{ $tf->user->name }} ||
                            ...
                            {{-- <div class="mb-4">
                                ...
                                <input type="text" name="total_price_wifi" id="total_price_wifi"
                                    ...
                                    value="{{ $transactionWifiItem->wifis->total_price_wifi }}"
                            ...
                            {{-- <div class="mb-4">
                                ...
                                <input type="date" name="expired_wifi" id="expired_wifi"
                                    ...
                                    value="{{ $transactionWifiItem->wifis->expired_wifi }}" required>
                            </div> --}}
                            {{-- <div class="mb-4">
                                ...
                                <select name="status" id="status"
                                    ...
                                    @foreach ($status_wifi as $item)
                                        <option value="{{ $item['value'] }}"
                                            {{ $item['value'] == old('status', $transactionWifiItem->wifis->status) ? 'selected' : '' }}>
                                            {{ $item['label'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4" id="nama-bank-input" style="display: none;">
                                ...
                                {{-- @foreach ($banks as $bank)
                                    @if ($bank->nama_bank == old('payment_bank', $transactionWifiItem->payment_bank))
                                        <p id="selected_bank">{{ $bank->nama_bank }}</p>
                                    @endif
                                @endforeach --}}
                                <div>
                            ...
                            <div class="mb-4" id="description">
                                ...
                                <textarea name="description" rows="4"
                                    ...
                                    placeholder="Tuliskan catatan..." value="{{ $transactionWifiItem->description }}">{{ $transactionWifiItem->description }}</textarea>
                            </div>
                            ...
                            <div class="mb-4" id="payment_status">
                                ...
                                <select name="payment_status"
                                    ...
                                    @foreach ($status_payment as $item)
                                        <option value="{{ $item['value'] }}"
                                            {{ $item['value'] == old('payment_status', $transactionWifiItem->payment_status) ? 'selected' : '' }}>
                                            {{ $item['label'] }}</option>
                                    @endforeach
                                </select>
