            ...
            <script>
                $(document).ready(function() {
                    ...
                    var table = $('#crudTable').DataTable({
                        ...
                        language: {
                            searchPlaceholder: "Search Data About",
                            decimal: ',',
                            thousands: '.',
                            paginate: {
                                first: "First",
                    ...
                    <div class="relative w-full h-full max-w-2xl md:h-auto">
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <div class="p-6 space-y-6">
                                <form action="{{ route('dashboard.about-utama.store') }}" method="POST"
                                    ...
                                    <div class="flex flex-wrap -mx-3 mb-6">
                                        <div class="w-full px-3">
                                            ...
                                            <input value="{{ old('title_about') }}" name="title_about"
                                                class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                                id="title_about" type="text" placeholder="Judul About">
                                        </div>
                                    </div>
                                    <div class="flex flex-wrap -mx-3 mb-6">
                                        <div class="w-full px-3">
                                            <label
                                                class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"
                                                for="description_about">
                                                Deskripsi About
                                            </label>
                                            <textarea name="description_about"
                                                class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                                id="description_about" type="text" placeholder="Tulis deskripsi">{{ old('description_about') }}</textarea>
                                        </div>
                                    </div>
                                    <div class="flex flex-wrap -mx-3 mb-6">
                                        <div class="w-full px-3">
                                            ...
                                            <input multiple accept="image/*" value="{{ old('image_about') }}"
                                                name="image_about"
        ...
        @push('javascript')
            <script>
                $('body').on('click', '.edit-button', function() {
                    ...
                    $.ajax({
                        url: "{{ route('dashboard.about-utama.edit', ['about_utama' => ':id']) }}".replace(':id',
                            about_id),
                        cache: false,
                        type: "GET",
                        success: function(response) {
                            window.location.href =
                                "{{ route('dashboard.about-utama.edit', ['about_utama' => ':id']) }}".replace(
                                    ':id',
                                    about_id) + '?_=' + new Date().getTime();
                        },
                        error: function() {
            ...
            <script>
                $(document).ready(function() {
                    $('body').on('click', '.delete-button', function() {
                        var about_id = $(this).data("id");
                        Swal.fire({
                            title: 'Apakah anda yakin ingin menghapus landing page about ini?',
                            icon: 'warning',
                            ...
                            if (result.isConfirmed) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ route('dashboard.about-utama.destroy', ':id') }}"
                                        .replace(
                                            ':id', about_id),
                                    data: {
                                        "_token": "{{ csrf_token() }}"
